    ...
    if (key.includes('instructions')) {
      return (
        <div css={{ display: 'grid'}}>
				<textarea
					css={{ height: '25px', padding: '4px', fontFamily: 'GuardianTextSans', fontSize: '1.0625rem', width: `${key.includes('stepNumber') ? '50px': '500px'}`, height: '50px', margin: '4px'}}
					key={key}
					id={key}
					// type={typeof value}
					// label={key}
		...
		return (
			<div css={{ display: 'grid'}}>
				<input
					css={{ height: '25px', padding: '8px', margin: '4px', fontFamily: 'GuardianTextSans', fontSize: '1.0625rem', width: `${typeof value === 'number' ? '50px' : key.includes('unit') ? '80px' : key.includes('ingredients') ? '250px' : '400px'}`, }}
					key={key}
					id={key}
					type={typeof value}
					label={key}
		...
		return (
			<>
				<select
					css={{ width: '200px', display: 'grid', fontFamily: 'GuardianTextSans', fontSize:'1.0625rem', marginRight: '4px'}}
					value={value}
					key={key}
					id={key}
