...
import {
  ...
  markupHtmlDuplication,
  markupHtmlDuplication2,
  ...
  const highlightInfo = [
    {
      id: "test",
      type: "test",
      range: { elementNumber: 0, startCharacter: 0, endCharacter: 35 },
    },
  ] as Highlight[];
  const [before, inside, after] = createHighlightTextFractions(
    highlightInfo,
    testIngListHTML,
  );
...
test("Mark HTML correctly", () => {
  ...
  // Correct markup text?
  expect(sel[0].firstChild.text.replace(/\s/g, "")).toEqual(
    expectedMarkupText.replace(/\s/g, ""),
  );
  // Correct text outside markup?
  expect(after[0].replace(/\s/g, "")).toEqual(
    expectedAfterMarkup.replace(/\s/g, ""),
  );
});
...
test("No duplication in ingredients fields ", () => {
  ...
  const [before, inside, after] = createHighlightTextFractions(
    highlightInfo,
    htmlNodes[0].innerHTML,
  );
  const merged = mergeHighlights(before, inside, after);
  expect(merged.replace(/\s/g, "")).toEqual(
    htmlNodes[0].innerHTML.replace(/\s/g, ""),
  );

  // Check 'createHighlightHTML' function as a whole
  const result = createHighlightHTML(highlightInfo, htmlNodes[0], {
    test: "yellow",
  });
  expect(result.replace(/\s/g, "")).toEqual(
    markupHtmlDuplication.replace(/\s/g, "").replace(/"/g, '"'),
  );
});
...
test("No duplication in ingredients fields with reverse order ", () => {
  ...
  expect(result.replace(/\s/g, "")).toEqual(
    markupHtmlDuplication2.replace(/\s/g, "").replace(/"/g, '"'),
  );
});
...
test("No duplication in ingredients field when skipping ingredient ", () => {
  ...
  const [before, inside, after] = createHighlightTextFractions(
    highlightInfo,
    htmlNodes[1].innerHTML,
  );
  const merged = mergeHighlights(before, inside, after);
  expect(merged.replace(/\s/g, "")).toEqual(
    htmlNodes[1].innerHTML.replace(/\s/g, ""),
  );

  // Check 'createHighlightHTML' function as a whole
  const result = createHighlightHTML(highlightInfo, htmlNodes[1], {
    test: "yellow",
  });
  expect(result.replace(/\s/g, "")).toEqual(
    markupHtmlDuplicationWithMissingIngredient
